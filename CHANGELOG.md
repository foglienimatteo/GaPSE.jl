...
- RELEASE: the version is `0.10.0`, a minor bump rather than a patch because this release
  changes behaviour that callers can observe: `MySpline` replaces `Dierckx` for every 1D
  interpolation and only supports `bc="error"` (so `spline_com_H` throws outside its range
  instead of clamping), `Δχ = 0` is no longer an error, the `Δχ → 0` branches use a
  threshold relative to the local distances, and the numerical results moved enough that 33
  reference files had to be regenerated. `Project.toml` gains a `[compat]` section - it had
  none - with a lower bound for each dependency and `julia = "1.12"`, and a `[workspace]`
  table listing `test`;

- the unit tests run on Julia 1.12 only. The 1.9 job is gone, together with the
  `continue-on-error` "forward-compatibility probe" that carried 1.12 as advisory: the
  matrix is now a single blocking entry per platform, `ubuntu-latest`/x64 (which is also
  where coverage is taken) and `macos-latest`/aarch64. The documentation workflow builds on
  1.12 as well, and the Julia badge of `README.md` and `docs/src/index.md` follows;

- the `Dockerfile` is rebuilt on the official `julia:1.12-bookworm` image. It used to start
  from `jupyter/julia-notebook:julia-1.9.1`, which cannot provide anything newer: that image
  has been frozen since October 2023 and its most recent Julia is 1.9.3. JupyterLab,
  Matplotlib and IJulia are installed explicitly, the extras the notebooks need (`Plots`,
  `LaTeXStrings`, `PyPlot`) go into the shared v1.12 environment rather than into
  `Project.toml`, `PYTHON` is pinned to the system interpreter so PyCall does not build its
  own, and the container runs as a non-root user and serves JupyterLab on 8888 as before;

- the hand-written tables of contents and section links of the Theory pages were written
  with GitHub-style anchors (`#why-the-cut-cannot-be-dropped`), which Documenter does not
  generate: its ids keep the case and the punctuation of the heading
  (`#Why-the-cut-cannot-be-dropped`). Every one of those links was therefore dead on the
  published manual while working on GitHub. They are now `@ref` links, which Documenter
  resolves and validates at build time, and the hand-written table of contents of
  `theory_IlnIntegrals.md` is a `@contents` block like the ones of the other pages;

- `docs/src/theory_InputPowerSpectrum.md`: the Poisson equation (P.3) carries the scale
  factor `a²(z) = (1+z)^-2`, not the growth factor `D(z)`. The two coincide only during
  matter domination, with the normalisation `D(z) = (1+z)^-1` used on that page, and at
  `z = 0` they differ by `D(0) ≃ 0.79`, so the substitution was a 1.6 factor in `P_m`. The
  derivation of (P.4) is also restored to the Friedmann equation for the total density,
  `H² = 8πG ρ_tot/3` with `Ω_M0 := ρ_m0/ρ_tot0`, which needs no Einstein-de Sitter
  assumption and makes the `(1+z)` of (P.5) come out as the three powers of the density
  minus the two of the comoving Laplacian. A duplicated half-sentence before (P.2) and the
  `\mathrm{d}d a` typo are fixed, and the power-law exponent of `InputPS` is written `ν`
  instead of `s`, which everywhere else on these pages is the comoving separation;

- the section of `theory_IlnIntegrals.md` about the two `k` ranges is rewritten as a
  statement of what each range is for, with the numbers behind the choice: the `I_l^n` need
  the wide `xicalc` grid `[1e-5, 1e3]`, while `σ_0` stands in for `I_0^0(Δχ_min)` in the
  limit branches and is therefore cut at `k_max ≈ 1/Δχ_min = 10`. Measured on
  `WideA_ZA_pk.dat`: `σ_0(<10) = 18.58` against `I_0^0(0.1) = 23.73`, i.e. 22% below the
  number it replaces, where the `xicalc` range would give `143.3`, a factor 6 above it;

- the two comments in `GNC_AutoLensing.jl` and `LD_AutoLensing.jl` recording that an earlier
  form of the Lensing-Lensing limit had its `σ_0` coefficient a factor 3 too large are
  removed: the three Lensing-Lensing integrands now carry the same comment, and the
  derivation and the mistake itself are in the manual;

- `docs/src/theory_Ilnintegrals-mellin.md` holds the large-`s` behaviour of the `I_l^n` and
  the Mellin-transform proof, moved out of `theory_IlnIntegrals.md`. It is deliberately
  absent from the `pages` list of `docs/make.jl`, so it is built and reachable by URL but
  has no entry in the navigation menu; the `@contents` block of `index.md` is given an
  explicit `Pages` list so it does not link it either, and `make.jl` drops its records from
  `search_index.js` after `makedocs`, Documenter having no per-page switch for that. The
  page carries a `!!! warning "Draft"` saying that the argument has not been reviewed to the
  standard of the rest of the manual;

- `Iln_terms.ipynb` is converted like the others: sections with *GaPSE setup*, *The I_l^n by direct quadrature*, *Plot functions* and *Saving functions* collapsed, results left open, and the plotting moved to the customisable API - `plot_kwargs(kwargs...)`, `hlabel` for the horizontal y labels, `logticks(lo, hi; step)` alongside the pre-existing minor-tick generator, now named `logticks_minor`, and every label, limit, scale and style a keyword with the rest forwarded. Saving is split out of the five plotting functions, which used to `savefig` internally and so could not be called just to look at a figure. The three `save_*_data` functions, which lived only in the deleted `Iln_terms.jl` and were CALLED but never DEFINED in the notebook, are restored into it: that cell could never have run as it stood;
...
- `theory_DeltaChiLimits.md` gains the section "What the `σ_i` depend on". Eq.(2.1a) is obtained by expanding `j_l(q Δχ)` and integrating term by term, which is legitimate because the `q` integral runs over the finite `[k_min, k_max]` of the definition: there `q Δχ ≤ k_max Δχ`, so the single condition `Δχ << 1/k_max` makes the argument small uniformly. Every `σ_i` is then an ordinary finite number. What does depend on the range is the *value* of `σ_0` and `σ_4`, and the section says which branches inherit that dependence (the three of Family 1 and the two of Family 3) and how `k_max` is chosen: the branch stands in for `I_0^0(Δχ_min)`, so the right cut is `k_max ≈ 1/Δχ_min = 10`, the `IPSTools` default;
...
- the plotting helpers of `theory/` are now customisable: `plot_kwargs(kwargs...)` merges overrides onto the shared defaults (last value wins), `logticks(lo, hi; step)` takes a step and also accepts the data directly, `hlabel` keeps the trailing-space hack that a horizontal `yguidefontrotation = -90` label needs in one place, and `vspec`/`vlines!` replace the old eighteen `r1/l1/ls1/lw1/c1/alpha1 ...` keywords with a single list of markers that a figure can override, shorten or extend. Every label, scale and tick range of a plot is a keyword, with the rest forwarded to `plot_kwargs`. Applied to all four analyses of `theory/`;

- CORRECTION, the `σ_i` are all finite, and `I_0^0` does have an `s → 0` limit. An earlier version of this branch wrote the definition of the `I_l^n` and of the `σ_i` with `∫_0^∞` and concluded that `σ_0` diverges in the ultraviolet, `σ_4` in the infrared, and that `I_0^0(s) ≈ σ_0(<1/s) ~ s^(-0.359)` has no limit. The step that produced it - replacing `j_l(qs)` by `(qs)^l` under the integral because `s → 0` - is not valid on an infinite range, since `q` runs to infinity and there is always a region `q > 1/s` where the replacement is false. What the code integrates is the finite `[k_min, k_max]` that `IPSTools` hands to `xicalc`, and there the replacement is licensed uniformly by `s << 1/k_max`. Measured directly over the same `[1e-5, 1e3]`, `I_0^0(s)/σ_0` is 0.166, 0.973, 0.99997 and 1.000000 at `s` = 1e-1, 1e-3, 1e-5 and 1e-6: a monotone approach to a finite limit, not a divergence. What remains true is that `σ_0` and `σ_4` *depend on their range* - `σ_0(<K) ~ K^0.359` with the fitted tail, logarithmically with the true CDM one - so neither can be quoted without it, while `σ_1`, `σ_2` and `σ_3` converge at both ends and do not care. `theory_IlnIntegrals.md` gains the section "Why the cut cannot be dropped", which splits `∫_0^∞` into `[0, k_min] + [k_min, k_max] + [k_max, ∞)` and shows that the outer pieces converge but scale as `s^(α-3)`, a different power of `s` from the `s^(l-n)` of the middle one - which is where the spurious divergence came from;
