...
- added `theory/input_ps.jl` and `theory/input_ps.ipynb`, and the new "The input Power Spectrum" page of the manual (`docs/src/InputPowerSpectrum.md`), which collects the asymptotic behaviour of `P(k)` at both ends, the extrapolation `InputPS` uses, and the convergence table above. The "The small-k slope of the matter Power Spectrum" section moved there out of `IlnIntegrals.md`, which now points to it; as a side effect that resolves a numbering collision, `(3.4)`, `(3.5)` and `(3.6)` having been used twice in that page;
