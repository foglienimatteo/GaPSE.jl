...
- NOTE: the `withF` reference files of `GNCxLD_SumXiMultipoles` and `LDxGNC_SumXiMultipoles` were regenerated *before* the guard above, so they still encode the corner bug and disagree with the corrected code by that same 1.5%. They need to be regenerated once more. The `noF` ones are fine: they were replacing values produced without any limit branch, i.e. the ill-conditioned direct evaluation of `J_22 ~ 1/Δχ^4`, which at `s = 500` disagreed with its own reference by 33% to 181%;
