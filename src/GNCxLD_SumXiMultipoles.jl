...
the Galaxy Number Counts (GNC) and the perturbations of the Luminosity Distance (LD)  
and their sum in the comoving distance `s1` and a comoving distance `s` from it 
for the input `cosmo::Cosmology`.
We remember that all the distances are measured in ``h_0^{-1}\\mathrm{Mpc}``.

This function makes a for-loop on the `GaPSE.GR_EFFECTS_GNCxLD` strings, calling 
`ξ_GNCxLD_multipole` for each of them. To each string corresponds pretty intuitively one of the 
20 GNCxLD effects. They are currently, in order:
...
This function recall internally `ξ_GNCxLD_multipole`, so the kwargs are the same; we report them
for comfortness:
...
- `kwargs...` : other keyword arguments that will be passed to ALL the 
  GNCxLD TPCF effect (`ξ_GNCxLD_Doppler`, `ξ_GNCxLD_Lensing`, ...); if one of them has that keyword argument,
...
- a `Vector{Float64}` with all the values of each ξ; they are ordered
  following `GR_EFFECTS_GNCxLD`


See also: [`integrand_ξ_GNCxLD_multipole`](@ref), [`ξ_GNCxLD_multipole`](@ref),
[`map_sum_ξ_GNCxLD_multipole`](@ref), [`print_map_sum_ξ_GNCxLD_multipole`](@ref),
...
Evaluate the multipole of order `L` of all the two-point cross correlation multipole between 
the Galaxy Number Counts (GNC) and the perturbations of the Luminosity Distance (LD) and their sum
...
This function makes a for-loop on the `GaPSE.GR_EFFECTS_GNCxLD` strings, calling 
`map_ξ_GNCxLD_multipole` for each of them. To each string corresponds pretty intuitively one of the 
20 GNCxLD effects. They are currently, in order:
...
This function recall internally `map_ξ_GNCxLD_multipole`, so the kwargs are the same; we report them
for comfortness:
...
See also: [`map_ξ_GNCxLD_multipole`](@ref),
[`sum_ξ_GNCxLD_multipole`](@ref), [`print_map_sum_ξ_GNCxLD_multipole`](@ref),
[`Cosmology`](@ref), [`GR_EFFECTS_GNCxLD`](@ref)
"""
function map_sum_ξ_GNCxLD_multipole(
     cosmo::Cosmology,
     ss=nothing;
     N_log::Int=1000,
     sum_xi::Bool=true,
     ...
     ALL = [
          begin
               _, xis = map_ξ_GNCxLD_multipole(cosmo, effect, v_ss;
                    sum_xi=sum_xi, specif_kwargs_GNCxLD(effect, kwargs)...)
               xis
...
See also: [`map_ξ_GNCxLD_multipole`](@ref),
[`sum_ξ_GNCxLD_multipole`](@ref), [`map_sum_ξ_GNCxLD_multipole`](@ref),
[`Cosmology`](@ref), [`GR_EFFECTS_GNCxLD`](@ref)
"""
function print_map_sum_ξ_GNCxLD_multipole(
     ...
     dir = length(split(out, "/")) == 1 ? "all_GNCxLD_standalones_CFs/" :
           join(split(out, "/")[begin:end-1] .* "/") * "all_GNCxLD_standalones_CFs/"
     ...
     t1 = time()
     ss, xis, ALL = map_sum_ξ_GNCxLD_multipole(cosmo, v_ss;
          L=L, s1=s_1, pr=pr, sum_xi=sum_xi, kwargs...)
     t2 = time()

     (pr) && println("\ntime needed for map_sum_ξ_GNCxLD_multipole L=$L" *
     ...
     open(out, "w") do io
          ...
          println(io, "#\n# This is an integration map on mu of the sum" *
                      " of all the ξ_GNCxLD L=$L multipole GR effects")
          ...
          elseif single == true
               println(io, "# 1: s [Mpc/h_0] \t 2: xi_SUM \t " *
                           join([string(i) for i in 3:length(GR_EFFECTS_GNCxLD)+2] .*
                                ": xi_" .* GR_EFFECTS_GNCxLD .* " \t "))
     ...
     if single == false
          for (effect, vec) in zip(GaPSE.GR_EFFECTS_GNCxLD, ALL)
               open(dir * "xi_GNCxLD_" * effect * "_L$L" * ".txt", "w") do io
