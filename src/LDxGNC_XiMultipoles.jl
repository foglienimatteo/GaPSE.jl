...
of the chosen two-point cross correlation between a perturbation of the Luminosity Distance (LD) effect
and a Galaxy Number Counts (GNC) effect  effect
...
In the former method you have to pass as an input the LDxGNC `effect` function you want 
to integrate, while in the (recommended) latter one it's necessary to specify the
name of the LDxGNC TPCF term among the following:
...
- `effect::Function` or `effect::String` : name/function you want to consider, among the ones
  inside `GaPSE.GR_EFFECTS_LDxGNC`/`GaPSE.VEC_ξs_LDxGNC` respectively.
...
[`y`](@ref), [`s2`](@ref), [`GR_EFFECTS_LDxGNC`](@ref), [`GaPSE.VEC_ξs_LDxGNC`](@ref)
"""
...
Evaluate the multipole of order `L` of the chosen two-point cross correlation 
between a perturbation of the Luminosity Distance (LD) and a Galaxy Number Counts (GNC)
effect term i.e. the following function ``\\xi_L(s_1, s)``:
...
- `kwargs...` : other keyword arguments that will be passed to the selected 
  LDxGNC TPCF effect (`ξ_LDxGNC_Doppler`, `ξ_LDxGNC_Lensing`, ...)
...
[`y`](@ref), [`s2`](@ref), [`GR_EFFECTS_LDxGNC`](@ref), [`VEC_ξs_LDxGNC`](@ref)
"""
ξ_LDxGNC_multipole
...
Evaluate the multipole of order `L` of the chosen two-point cross correlation 
between a perturbation of the Luminosity Distance (LD) and a Galaxy Number Counts (GNC)
effect term for all the comoving distance 
...
function map_ξ_LDxGNC_multipole(cosmo::Cosmology,
     ...
     t2 = time()
     (!sum_xi) && (pr) && println("\ntime needed for map_ξ_LDxGNC_multipole for $effect, L=$L " *
...
Evaluate the multipole of order `L` of the of the chosen two-point cross correlation 
between a perturbation of the Luminosity Distance (LD) and a Galaxy Number Counts (GNC) 
effect term for all the  comoving distance 
