...
This function makes a for-loop on the `GaPSE.GR_EFFECTS_LD` strings, calling 
...
This function recall internally `ξ_LD_multipole`, so the kwargs are the same; we report them
for comfortness:
...
- `kwargs...` : other keyword arguments that will be passed to ALL the 
  LD TPCF effect (`ξ_LD_Doppler`, `ξ_LD_Lensing`, ...); if one of them has that keyword argument,
...
function map_sum_ξ_LD_multipole(
     ...
     ALL = [
          begin
               _, xis = map_ξ_LD_multipole(cosmo, effect, v_ss;
                    sum_xi=sum_xi, specif_kwargs_LD(effect, kwargs)...)
               xis
...
function print_map_sum_ξ_LD_multipole(
     ...
     dir = length(split(out, "/")) == 1 ? "all_LD_standalones_CFs/" :
           join(split(out, "/")[begin:end-1] .* "/") * "all_LD_standalones_CFs/"
     ...
     (pr) && println("\ntime needed for map_sum_ξ_LD_multipole L=$L" *
     ...
     open(out, "w") do io
          ...
          println(io, "#\n# This is an integration map on mu of the sum" *
                      " of all the ξ_LD L=$L multipole GR effects")
     ...
     if single == false
          for (effect, vec) in zip(GaPSE.GR_EFFECTS_LD, ALL)
               open(dir * "xi_LD_" * effect * "_L$L" * ".txt", "w") do io
