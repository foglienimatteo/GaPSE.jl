...
function integrand_ξ_GNCxLD_LocalGP_Lensing(
    ...
    JI_sum = if Δχ2 ≥ min(Δχ_min, Δχ_min * max(χ2, s1))
...
Lensing one arising from the Luminosity Distance (LD) perturbations.
...
function ξ_GNCxLD_LocalGP_Lensing(s1, s2, y, cosmo::Cosmology;
    ...
    int_ξs = [
      en * GaPSE.integrand_ξ_GNCxLD_LocalGP_Lensing(IP, P1, P2, y, cosmo; kwargs...)
      for IP in IPs
    ]
...
effect arising from the Luminosity Distance (LD) perturbations and Local
Gravitational Potential (GP) one arising from the Galaxy Number Counts (GNC).

It's computed through the symmetric function `ξ_GNCxLD_LocalGP_Lensing`; check its documentation for
