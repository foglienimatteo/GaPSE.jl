...
Galaxy Number Counts (GNC) and the Integrated Gravitational Potential (GP)
...
This function is used inside `ξ_GNCxLD_Lensing_IntegratedGP` with the trapz() from the 
...
function ξ_GNCxLD_Lensing_IntegratedGP(
    ...
    int_ξs = [
        en * GaPSE.integrand_ξ_GNCxLD_Lensing_IntegratedGP(IP1, IP2, P1, P2, y, cosmo; kwargs...)
...
It's computed through the symmetric function `ξ_GNCxLD_Lensing_IntegratedGP`; check its documentation for
...
[`ξ_GNCxLD_Lensing_IntegratedGP`](@ref)
"""
function ξ_LDxGNC_IntegratedGP_Lensing(s1, s2, y, cosmo::Cosmology; 
    b1=nothing, b2=nothing, s_b1=nothing, s_b2=nothing,
    ...
    ξ_GNCxLD_Lensing_IntegratedGP(s2, s1, y, cosmo; 
