...
function integrand_ξ_GNCxLD_IntegratedGP_Doppler(
    ...
    I13 = Δχ1 ≥ min(Δχ_min, Δχ_min * max(s2, χ1)) ? cosmo.tools.I13(Δχ1) : zero(Δχ1)
...
This function is used inside `ξ_GNCxLD_IntegratedGP_Doppler` with the trapz() from the 
...
It's computed through the symmetric function `ξ_GNCxLD_IntegratedGP_Doppler`; check its documentation for
...
[`ξ_GNCxLD_IntegratedGP_Doppler`](@ref)
"""
