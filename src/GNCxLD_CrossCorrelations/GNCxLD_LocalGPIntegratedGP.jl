...
Integrated GP one arising from the Luminosity Distance (LD) perturbations.
...
This function is used inside `ξ_GNCxLD_LocalGP_IntegratedGP` with the trapz() from the 
...
It's computed through the symmetric function `ξ_GNCxLD_LocalGP_IntegratedGP`; check its documentation for
...
[`ξ_GNCxLD_LocalGP_IntegratedGP`](@ref)
"""
