...
```math
\\begin{split}
   \\xi^{\\phi v_{\\parallel}} ( s_1 , s_2, y ) = 
    D_1 D_2 \\,\\mathfrak{J}^{\\phi v_{\\parallel}}_{\\alpha}
    \\left[ 
        \\frac{1}{90} I_0^0 (s) +
        \\frac{1}{63} I_2^0 (s) + 
        \\frac{1}{210} I_4^0 (s) +
...
```math
\\begin{split}
   \\mathfrak{J}^{\\phi v_{\\parallel}}_{\\alpha} &=
    - \\frac{f_2 \\mathcal{H}_2 \\mathfrak{R}_2 s^2}{a_1} (y s_1 - s_2) 
    \\times\\\\
    &\\qquad\\qquad\\qquad
    \\left[ 
...
It's computed through the symmetric function `ξ_GNCxLD_LocalGP_Doppler`; check its documentation for
