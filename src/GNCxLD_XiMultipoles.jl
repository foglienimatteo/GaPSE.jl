...
of the chosen two-point cross correlation between a Galaxy Number Counts (GNC) effect and a
perturbation of the Luminosity Distance (LD) 
...
In the former method you have to pass as an input the GNCxLD `effect` function you want 
...
- `effect::Function` or `effect::String` : name/function you want to consider, among the ones
  inside `GaPSE.GR_EFFECTS_GNCxLD`/`GaPSE.VEC_ξs_GNCxLD` respectively.
...
between a Galaxy Number Counts (GNC) effect and a perturbation of the Luminosity Distance (LD) 
...
- `kwargs...` : other keyword arguments that will be passed to the selected 
  GNCxLD TPCF effect (`ξ_GNCxLD_Doppler`, `ξ_GNCxLD_Lensing`, ...)
...
[`y`](@ref), [`s2`](@ref), [`GR_EFFECTS_GNCxLD`](@ref), [`VEC_ξs_GNCxLD`](@ref)
"""
ξ_GNCxLD_multipole
...
Evaluate the multipole of order `L` of the chosen two-point cross correlation 
between a Galaxy Number Counts (GNC) effect and a perturbation of the Luminosity Distance (LD) 
term for all the comoving distance 
...
function print_map_ξ_GNCxLD_multipole(
    ...
    s_1 = isnothing(s1) ? cosmo.s_eff : s1
    t1 = time()
    vec = map_ξ_GNCxLD_multipole(cosmo, effect, ss; s1=s_1, L=L, kwargs...)
    t2 = time()
