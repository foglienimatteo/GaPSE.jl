...
@testset "test sum_ξ_LDxGNC_multipole no_window" begin
     table = readdlm("datatest/GNCxLD_SumXiMultipoles/map_sum_xi_LDxGNC_L0_noF_specific_ss.txt", comments = true)
     ...
     @testset "s = 10, L = 0, no_window" begin
          ...
          calc_res_sum, calc_res_xis = GaPSE.sum_ξ_LDxGNC_multipole(COSMO.s_eff, s, COSMO; kwargs...)
...
@testset "test sum_ξ_LDxGNC_multipole with_window" begin
     table = readdlm("datatest/GNCxLD_SumXiMultipoles/map_sum_xi_LDxGNC_L0_withF_specific_ss.txt", comments = true)
     ss = convert(Vector{Float64}, table[:, 1])
     all_res_sums = convert(Vector{Float64}, table[:, 2])
     all_res_xis = [convert(Vector{Float64}, table[:, i]) for i in 3:22]
     RTOL = 1e-2
...
@testset "test map_sum_ξ_LDxGNC_multipole no_window" begin
     ...
     calc_ss, calc_sums, calc_xis = GaPSE.map_sum_ξ_LDxGNC_multipole(COSMO, ss; kwargs...)
...
@testset "test print_map_sum_ξ_LDxGNC_multipole no_window" begin
     table = readdlm("datatest/GNCxLD_SumXiMultipoles/map_sum_xi_LDxGNC_L0_noF_specific_ss.txt", comments = true)
     ss = convert(Vector{Float64}, table[:, 1])
     res_sums = convert(Vector{Float64}, table[:, 2])
     res_xis = [convert(Vector{Float64}, table[:, i]) for i in 3:18]
     ...
     @testset "first" begin
          name = "first_map_sum_xi_LDxGNC_L0_noF_specific_ss.txt"
          isfile(name) && rm(name)

          GaPSE.print_map_sum_ξ_LDxGNC_multipole(COSMO, name, ss;
               single = true, kwargs...)
     ...
     @testset "second" begin
          name = "second_map_sum_xi_LDxGNC_L0_noF_specific_ss.txt"
          isfile(name) && rm(name)
          ...
          calc_xis = [
               begin
                    a_name = "all_LDxGNC_standalones_CFs/xi_LDxGNC_" * effect * "_L0" * ".txt"
          ...
          for effect in GaPSE.GR_EFFECTS_LDxGNC
               rm("all_LDxGNC_standalones_CFs/xi_LDxGNC_" * effect * "_L0" * ".txt")
          end
