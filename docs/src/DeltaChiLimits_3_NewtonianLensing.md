...
!!! warning "A typo in the docstring of `integrand_ξ_GNC_Newtonian_Lensing`"
    The docstring of that function writes the ``b_1`` part of ``J^{\delta\kappa}_{02}`` as
    ``\left[(9y^2+11) f_1 - 7(y^2+3) b_1\right] s_1^2 \chi_2``, with a **minus** sign, while
    the code has a plus. The difference is ``14 \, b_1 \chi_2 s_1^2 (y^2+3)``, and it matters:
    with the docstring sign ``N_{02}^{(b)}`` would **not** vanish at the singular point
    ...
    ``- b_1 s_1^3 \sigma_{-2}/6``. The code is the correct one — every other ``J`` of this
    family is built to vanish at the singular point — so the docstring should be fixed.
